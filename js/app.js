 ...
 */
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".gallery-slider__slider")) new core(".gallery-slider__slider", {
                modules: [ Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
        ...
        if (document.querySelector(".slider-story__slider")) new core(".slider-story__slider", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
...
***************************************************************************** */
        ...
        const galleryTwo = document.querySelectorAll("[data-gallery-two]");
        if (galleries.length) {
            let galleyItems = [];
        ...
        } else if (galleryTwo.length) {
            let galleryItems = [];
            galleryTwo.forEach((galleryT => {
                galleryItems.push({
                    galleryT,
                    galleryTClass: lightgallery_es5(galleryT, {
                        plugins: [ lg_zoom_min, lg_thumbnail_min ],
                        licenseKey: "7EC452A9-0CFD441C-BD984C7C-17C8456E",
                        speed: 800,
            ...
            flsModules.galleryT = galleryItems;
        }
        ...
        da.init();
        const videoFile = document.querySelector(".video__item");
        const videoPlay = document.querySelector(".video__play");
        document.addEventListener("click", (function(e) {
            const targetElement = e.target;
            if (videoFile) if (targetElement.classList.contains("video__item") || targetElement.classList.contains("video__play")) if (videoFile.paused) {
                videoFile.play();
                videoPlay.classList.add("play-hidden");
            } else {
                videoFile.pause();
                videoPlay.classList.remove("play-hidden");
            }
            e.preventDefault;
        }));
        let script_link = document.getElementsByClassName("menu__link");
        let url = document.location.href;
        for (let i = 0; i < script_link.length; i++) if (url == script_link[i].href) script_link[i].classList.add("active-menu");
        window["FLS"] = true;
        isWebp();
        menuInit();
        spollers();
